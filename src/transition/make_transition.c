...
{
    if (myrpg->transition_state == 1) {
        sfView_zoom(GAME_INFO->map_view, 0.8);
        if (sfView_getSize(GAME_INFO->map_view).y <= 10) {
            myrpg->transition_state = 2;
        }
    }
    if (myrpg->transition_state == 2) {
...
int make_transition(myrpg_t *myrpg)
{
    ...
    if (myrpg->transition_state == 3)
        return 1;
    if (myrpg->transition_state == 0) {
        clock = sfClock_create();
        myrpg->transition_state = 1;
    }
