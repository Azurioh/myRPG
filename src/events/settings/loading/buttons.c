...
static void init_others_button(button_t **buttons, settings_t *settings)
{
    char *check_fullscreen = my_strdup("assets/more.png");
    ...
    sfSprite_setScale(buttons[9]->image_sprite,
        (sfVector2f){settings->scaling / 1.8, settings->scaling / 1.8});
    buttons[8]->initial_scaling = settings->scaling;
    buttons[9]->initial_scaling = settings->scaling / 1.8;
}
...
button_t **load_settings_buttons(settings_t *settings)
{
    ...
    init_others_button(buttons, settings);
    buttons[10] = init_button("quit",
        (sfVector2f){960 * settings->scaling, 860 * settings->scaling},
        "assets/quit-button.png", &close_settings);
    buttons[10]->initial_scaling = settings->scaling / 1.5;
    sfSprite_setScale(buttons[10]->image_sprite,
