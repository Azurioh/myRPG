...
{
    int fd = open("save/.settings", O_CREAT | O_WRONLY | O_TRUNC, 0666);
...
void save_player(myrpg_t *myrpg)
{
    ...
    fd = open("save/.player", O_CREAT | O_WRONLY | O_TRUNC, 0666);
    if (fd == -1) {
...
void save_inventory(myrpg_t *myrpg)
{
    ...
    if (!INVENTORY)
        return;
    fd = open("save/.inventory", O_CREAT | O_WRONLY | O_TRUNC, 0666);
    if (fd == -1) {
...
void save_quests(myrpg_t *myrpg)
{
    ...
    if (!INVENTORY)
        return;
    fd = open("save/.quests", O_CREAT | O_WRONLY | O_TRUNC, 0666);
    if (fd == -1) {
